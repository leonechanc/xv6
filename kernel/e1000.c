...
e1000_transmit(struct mbuf *m)
{
  ...
  //
  acquire(&e1000_lock);
  // First ask the E1000 for the TX ring index at 
  // which it's expecting the next packet, by reading the E1000_TDT control register.
  ...
  // and m->len is the packet length. Set the necessary cmd 
  // flags (look at Section 3.3 in the E1000 manual) 
  // and stash away a pointer to the mbuf for later freeing.
  desc->addr = (uint64)m->head;
  desc->length = m->len;
  desc->cmd = E1000_TXD_CMD_RS | E1000_TXD_CMD_EOP;
  tx_mbufs[idx] = m; // for cleaning 
...
e1000_recv(void)
{
  ...
  //
  // in e1000_recv we need to read all the packet at once rather than one by one
  // there's no need to add locks for e1000_intr will turn off the interrupt
  while (1) {
    // First ask the E1000 for the ring index at which the next waiting received 
    // packet (if any) is located, by fetching the E1000_RDT control register 
